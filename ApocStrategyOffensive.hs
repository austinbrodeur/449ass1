...
offensive board Normal White = 
     let validMoves = (checkKillMoves board White (getPieces (theBoard board) WK 0 0 0 0 4 4)) in
     if (validMoves /= [])
        then return $ Just [((head validMoves))]
        else let validMoves = (checkKillMoves board White (getPieces (theBoard board) WP 0 0 0 0 4 4)) in
             if (validMoves /= [])
                then return $ Just [((head validMoves))]
...
offensive board Normal Black =
     let validMoves = (checkKillMoves board Black (getPieces (theBoard board) BK 0 0 0 0 4 4)) in
     if (validMoves /= [])
        then return $ Just [((head validMoves))]
        else let validMoves = (checkKillMoves board Black (getPieces (theBoard board) BP 0 0 0 0 4 4)) in
             if (validMoves /= [])
                then return $ Just [((head validMoves))]
...
checkKillMoves board player (op:ops) = if ((getFromBoard (theBoard board) op == WK) || (getFromBoard (theBoard board) op == BK))
                                          then (checkKill board player (fromIntegerPairList knightOffset) op) ++ (checkKillMoves board player ops)
...
getEmptyMoves board White = (goodMoves board (fromIntegerPairList knightOffset) (getPieces (theBoard board) WK 0 0 0 0 4 4)) ++ (goodMoves board [(1, 1)] (getPieces (theBoard board) WP 0 0 0 0 4 4))
getEmptyMoves board Black = (goodMoves board (fromIntegerPairList knightOffset) (getPieces (theBoard board) BK 0 0 0 0 4 4)) ++ (goodMoves board [(1, 1)] (getPieces (theBoard board) BP 0 0 0 0 4 4))
